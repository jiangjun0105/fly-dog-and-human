...
>
> Every critical finding below has been **verified by reading the actual source code**.
...
> All findings across the PR in a single table, ordered by severity (critical → warning → nit).
...
> If no findings: "No issues found. The changes are clean and well-structured."
...
> Cross-reference each acceptance criterion from the task file. If the PR is not task-driven, omit this section.

| #   | Criterion | Verified? | Evidence / Notes |
| --- | --------- | --------- | ---------------- |
| 1   | [AC text] | yes / no  | [how verified]   |

______________________________________________________________________

## Design Doc Alignment

> If this PR implements a feature with a design doc (in `docs/architecture/` or `docs/design/`), note whether the implementation matches the design. If no design doc exists, omit this section.

- **Design doc**: [path or "N/A"]
- **Alignment**: [matches / deviates — explain any intentional deviations]
